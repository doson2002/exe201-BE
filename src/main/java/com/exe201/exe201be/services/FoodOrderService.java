...
public class FoodOrderService implements IFoodOrderService{
    ...
    public FoodOrder createOrder(List<OrderRequestDTO> orderRequests,
        ...
        for (OrderRequestDTO orderRequest : orderRequests){
            ...
            var newInventoryQuantity = foodItem.getInventoryQuantity() - orderRequest.getQuantity();
            foodItem.setInventoryQuantity(newInventoryQuantity);
            var newSoldQuantity = foodItem.getQuantitySold() + orderRequest.getQuantity();
            foodItem.setQuantitySold(newSoldQuantity);
    ...
    public Page<FoodOrder> getFoodOrdersByUserIdPaging(Long userId, String status, Date startDate, Date endDate, Pageable pageable) {
        return foodOrderRepository.findByUserIdAndStatusAndDateRange(userId, status, startDate, endDate, pageable);
    }
